...
def validate_registration_input(name, email, phone):
    ...
    if not (len(phone_clean) == 10) or phone_clean.startswith(def_phone):
...
for i in range(len(raw_registers)):
    ...
    print(f"""[{i+1}] {name} | Email: {email} | SDT: {phone} -> {email_status if email_status else "Trạng thái: HỢP LỆ"} | {phone_status if phone_status else "Trạng thái: HỢP LỆ"}""")
    ...
    try:
        ...
        if is_vip:
            if discount_code == "VIP10":
                ...
                total *= 0.8
        total *= 1.1  # Cộng thuế VAT 10%
        return total
    except ValueError:
        # Trong except ValueError: In ra thông báo bỏ qua đơn hàng bị lỗi số tiền.
        print(f"Xử lí lỗi {order_id}: Số tiền: '{raw_total}' không hợp lệ! Bỏ qua đơn hàng")

print("BÁO CÁO XỬ LÝ HÓA ĐƠN AN TOÀN (TRY-EXCEPT & VAT)")
for order in orders:
    processed_total = safe_process_invoice(order["id"], order["total"], order["discount_code"], order["is_vip"])
    if processed_total is not None:
        print(f"[{order['id']}] Tiền hàng: {order['total']} | CK ({order['discount_code']}): {order['discount_code']} | VAT 10%: {processed_total - float(order["total"])} -> Tổng: {processed_total:.2f} VNĐ")